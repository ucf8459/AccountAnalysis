...
def get_live_dashboard_data():
    """Development endpoint to get live dashboard data without authentication."""
    try:
        period_type = request.args.get('period_type', 'ytd')
        ...
        try:
            ...
            cursor.execute("""
                SELECT 
                    ...
                    COALESCE(SUM(net_profit), 0) as income
                FROM analytics.financial_summary
                WHERE period_start >= %s AND period_end <= %s
                GROUP BY EXTRACT(MONTH FROM period_start), TO_CHAR(period_start, 'Mon')
                ORDER BY month
            """, (start_date.date(), end_date.date()))
            ...
            if financial_data[0] == 0 and len(monthly_revenue) == 0:
...
def get_ai_summary():
    """Development endpoint to get AI-powered summary analysis without authentication."""
    try:
        from datetime import datetime, timedelta
        ...
        try:
            # Query comprehensive financial data for AI analysis
            cursor.execute("""
                SELECT 
                    COALESCE(SUM(total_revenue), 0) as total_revenue,
                    COALESCE(SUM(total_cogs), 0) as total_cogs,
            ...
            cursor.execute("""
                SELECT 
                    territory,
                    COALESCE(SUM(total_revenue), 0) as territory_revenue,
                    COALESCE(SUM(net_profit), 0) as territory_profit,
                    COUNT(DISTINCT account_id) as territory_accounts
                FROM analytics.financial_summary
                WHERE period_start >= %s AND period_end <= %s
                GROUP BY territory
            ...
            # Query account performance
            cursor.execute("""
                SELECT 
                    account_id,
                    COALESCE(SUM(total_revenue), 0) as account_revenue,
                    COALESCE(SUM(net_profit), 0) as account_profit,
                    COUNT(*) as transaction_count
                FROM analytics.financial_summary
                WHERE period_start >= %s AND period_end <= %s
                GROUP BY account_id
                ORDER BY account_profit DESC
                LIMIT 20
            """, (start_date.date(), end_date.date()))
            
            account_data = cursor.fetchall()
            
            # If no real data, use mock data for analysis
            if financial_data[0] == 0:
                # Use mock data for AI analysis
                financial_summary = {
                    'total_revenue': 3350126.0,
                    'total_cogs': 730094.0,
                    'total_expenses': 1870000.0,
            ...
            gross_margin = (financial_summary['gross_profit'] / financial_summary['total_revenue'] * 100) if financial_summary['total_revenue'] > 0 else 0
            net_margin = (financial_summary['net_profit'] / financial_summary['total_revenue'] * 100) if financial_summary['total_revenue'] > 0 else 0
            avg_revenue_per_account = financial_summary['total_revenue'] / financial_summary['num_accounts'] if financial_summary['num_accounts'] > 0 else 0
            
            # Create AI prompt
            prompt = f"""
            You are a financial analyst for a healthcare technology company. Analyze the following financial data and provide insights.
            
            Financial Data for {period_type.upper()} ({start_date.strftime('%B %Y')} to {end_date.strftime('%B %Y')}):
            
            SUMMARY:
            - Total Revenue: ${financial_summary['total_revenue']:,.2f}
            - Total COGS: ${financial_summary['total_cogs']:,.2f}
            ...
            - Number of Accounts: {financial_summary['num_accounts']:,}
            - Gross Margin: {gross_margin:.1f}%
            - Net Margin: {net_margin:.1f}%
            - Average Revenue per Account: ${avg_revenue_per_account:,.2f}
            
            TERRITORY PERFORMANCE:
            {chr(10).join([f"- {t['name']}: ${t['revenue']:,.2f} revenue, ${t['profit']:,.2f} profit, {t['accounts']} accounts" for t in analysis_data['territories'][:5]])}
            
            TOP ACCOUNTS:
            {chr(10).join([f"- Account {a['account_id']}: ${a['revenue']:,.2f} revenue, ${a['profit']:,.2f} profit" for a in analysis_data['top_accounts'][:5]])}
            
            Please provide a comprehensive analysis including:
            
            1. **PROS** (What's going well):
               - Identify strengths and positive trends
               - Highlight successful territories and accounts
               - Note any improvements or positive metrics
            
            2. **CONS** (Areas of concern):
            ...
            3. **ACTION SUGGESTIONS**:
               ...
               - Focus on key accounts that need attention
               - Suggestions for improving performance
               - Priority actions for management
            
            4. **KEY METRICS TO WATCH**:
               - Important KPIs to monitor
               - Thresholds or targets to set
               - Early warning indicators
            
            Format your response as a structured analysis with clear sections. Be specific, actionable, and professional.
            """
            
            # Try to call OpenAI API, fallback to mock analysis if no API key
            try:
                ...
                response = client.chat.completions.create(
                    model=OPENAI_MODEL,
                    messages=[
                        {"role": "system", "content": "You are an expert financial analyst specializing in healthcare technology companies. Provide clear, actionable insights based on financial data."},
                ...
                # Fallback to mock AI analysis when API is not available
                ai_analysis = f"""
**PROS** (What's going well):
• Strong revenue performance with ${financial_summary['total_revenue']:,.2f} in total revenue
• Healthy gross margin of {gross_margin:.1f}% indicating good cost control
• Net profit of ${financial_summary['net_profit']:,.2f} shows solid profitability
• Large account base of {financial_summary['num_accounts']:,} accounts provides stability
• Average revenue per account of ${avg_revenue_per_account:,.2f} is above industry average

**CONS** (Areas of concern):
• Net margin of {net_margin:.1f}% could be improved through expense optimization
• High total expenses of ${financial_summary['total_expenses']:,.2f} may need review
• COGS represents {financial_summary['total_cogs']/financial_summary['total_revenue']*100:.1f}% of revenue, which could be optimized

**ACTION SUGGESTIONS**:
• **Immediate Priority**: Review expense structure to improve net margin
• **Key Accounts Focus**: Identify top 20% of accounts by revenue and develop retention strategies
• **Cost Optimization**: Analyze COGS components for potential savings
• **Territory Performance**: Review underperforming territories and provide additional support
• **Account Growth**: Develop strategies to increase average revenue per account

**KEY METRICS TO WATCH**:
• Net margin target: 25% (currently {net_margin:.1f}%)
• Gross margin target: 80% (currently {gross_margin:.1f}%)
• Revenue per account target: $3,000 (currently ${avg_revenue_per_account:,.2f})
• Account retention rate: Monitor monthly for any declines
• Territory performance variance: Flag territories >20% below average

*Note: This is a demo analysis. Connect your OpenAI API key for real-time AI insights.*
                """
                source = source + '_mock_ai'
